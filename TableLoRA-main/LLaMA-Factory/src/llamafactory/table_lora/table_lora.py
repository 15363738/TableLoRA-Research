...
# except ImportError:
#     from peft.tuners.xlora import XLoraModel, XLoraConfig
# === Fix for missing XLoraModel ===
class XLoraModel: 
    pass
...
class LoraLayer_new(BaseTunerLayer):
    ...
    # === 在 LoraLayer_new 类中添加此方法 ===
    def _move_adapter_to_device_of_base_layer(self, adapter_name, device=None, dtype=None):
        ...
        # 移动 TableLoRA 特有权重 (关键步骤！)
        if self.is_tab_lora:
             if adapter_name in self.lora_A_tab_col:
                 self.lora_A_tab_col[adapter_name].to(device)
             if adapter_name in self.lora_A_tab_row:
                 self.lora_A_tab_row[adapter_name].to(device)
             if adapter_name in self.lora_B_tab:
                 self.lora_B_tab[adapter_name].to(device)

        # 移动 Embedding LoRA 权重
        if adapter_name in self.lora_embedding_A:
            self.lora_embedding_A[adapter_name].to(device)
            self.lora_embedding_B[adapter_name].to(device)
    # ==========================================
    def update_layer(
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        kwargs = {
            ...
            # === 修改开始 ===
            # 安全访问 runtime_config，如果不存在则设为 False
            "ephemeral_gpu_offload": getattr(lora_config, "runtime_config", None).ephemeral_gpu_offload if getattr(lora_config, "runtime_config", None) else False,
            # === 修改结束 ===
...
def _validate_model_kwargs_new(self, model_kwargs):
    # 创建副本，剔除我们自定义的参数，防止原逻辑报错
    model_kwargs_copy = model_kwargs.copy()
    model_kwargs_copy.pop("row_ids", None)
    model_kwargs_copy.pop("col_ids", None)
    
    # 这里的逻辑是：Transformers 会检查 model_kwargs_copy 里的参数是否合法
    # 既然我们已经把造成报错的参数拿掉了，剩下的参数（如 attention_mask）就能通过验证
    # 注意：这里我们简单地跳过具体验证调用，或者您可以尝试调用父类方法（如果版本兼容）
    # 为了最强兼容性，直接 pass 是最不坏的选择（毕竟我们知道自己在做什么）
    pass

# 应用补丁
generation_utils.GenerationMixin._validate_model_kwargs = _validate_model_kwargs_new


# 2. 覆盖 Llama 的输入准备函数：确保生成循环中 row_ids 被传递
original_prepare_inputs = LlamaForCausalLM.prepare_inputs_for_generation

def prepare_inputs_for_generation_new(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs):
    # 获取原始的标准输入
    model_inputs = original_prepare_inputs(self, input_ids, past_key_values, attention_mask, inputs_embeds, **kwargs)
    
    # 手动把 row_ids 和 col_ids 塞回去
    if "row_ids" in kwargs:
        row_ids = kwargs["row_ids"]
        # 如果正在使用 KV Cache (past_key_values不为空)，说明是在生成第2、3...个词
        # 此时 input_ids 长度为1，我们需要把 row_ids 也切片成对应长度，防止形状不匹配
        if past_key_values:
             # 取最后一位，这通常意味着生成的 token 继承上一个 token 的位置属性
             # (具体的切片逻辑取决于 TableLoRA 对生成部分的设计，这是通用的做法)
            row_ids = row_ids[:, -input_ids.shape[1]:]
        model_inputs["row_ids"] = row_ids
        
    if "col_ids" in kwargs:
        col_ids = kwargs["col_ids"]
        if past_key_values:
            col_ids = col_ids[:, -input_ids.shape[1]:]
        model_inputs["col_ids"] = col_ids
        
    return model_inputs

# 应用补丁
LlamaForCausalLM.prepare_inputs_for_generation = prepare_inputs_for_generation_new
...
def load_table_lora():
    ...
    LoraLayer._move_adapter_to_device_of_base_layer = LoraLayer_new._move_adapter_to_device_of_base_layer
